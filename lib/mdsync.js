...
export function buildMarkdown(note) {
  const segments = note.body ? [] : note.transcript?.segments || [];
  ...
  if (note.body) {
    lines.push("## Text", "", demote(note.body), "");
  } else {
...
export function syncNoteFile(note) {
  if (!note?.transcript?.segments?.length && !note?.body) return note?.mdPath || null;
  ...
  // name that is free or verifiably ours — never a file owned by another
  // note, and never the reading list's reserved filename.
  let rel = path.join(relDir, `${base}.md`);
  let n = 2;
  for (;;) {
    const reserved = rel === "reading-list.md";
    if (!reserved && !fs.existsSync(path.join(TRANSCRIPTS_DIR, rel))) break;
    if (!reserved && fileOwnedBy(rel, note.id) === true) break;
