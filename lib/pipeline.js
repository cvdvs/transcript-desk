...
export async function claudeCall(prompt, model, timeoutMs = 240000) {
  ...
  // covers both "never logged in" and "session expired" (different wording)
  if (/not logged in|\/login|re-?authenticate|oauth|401|invalid api key|token has expired/i.test(combined)) {
    return {
      ok: false,
      reason:
        "Claude needs a sign-in on this Mac — sessions expire every few weeks. Open Terminal, type `claude`, then `/login`, and follow the browser prompt. Transcripts, search, and exports keep working without it.",
    };
  }
