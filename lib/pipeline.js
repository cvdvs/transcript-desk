...
import { DATA_DIR, MEDIA_DIR, getNote, patchNote, saveNote, newId, markActive, markDone, cleanupMedia } from "./store";
...
async function claudeCall(prompt, model, timeoutMs = 240000) {
...
export async function summarize(id) {
  ...
  const { text, truncated } = transcriptToText(note.transcript.segments);
  const result = await claudeCall(summaryPrompt(note, text), SUMMARY_MODEL);
  if (result.ok) {
    patchNote(id, {
      summaryNote: null,
      summary: {
...
export async function generateTranslation(id) {
  const note = getNote(id);
  if (!note?.transcript?.segments?.length) return null;
  if (note.translationStatus === "running") return note; // in flight — the UI polls
  patchNote(id, { translationStatus: "running" });
  try {
    const { text, truncated } = transcriptToText(note.transcript.segments);
    const result = await claudeCall(translatePrompt(note, text), SUMMARY_MODEL, 420000);
...
export async function generateResearchPack(id) {
  ...
  try {
    const { text } = transcriptToText(note.transcript.segments);
    const result = await claudeCall(researchPrompt(note, text), RESEARCH_MODEL, 420000);
    if (!result.ok) throw new Error(result.reason);
    const updated = patchNote(id, {
      researchStatus: null,
